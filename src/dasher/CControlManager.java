	...
	private class CContNode extends CDasherNode {
		...
		private void PopulateChildren(CContNode existing) {
			...
			for (int i=0; i<actions.size(); i++) {
				long next = (i+1) * iNorm / actions.size();
		...
		public CDasherNode RebuildParent() {
			if (root.successors().indexOf(act)!=-1) {
				CContNode c = GetRoot(null, getOffset(), 0, GetLongParameter(Elp_parameters.LP_NORMALIZATION));
				c.PopulateChildren(this);
				return c;
			}
			return m_pNCMgr.getAlphabetManager().GetRoot(null, 0, GetLongParameter(Elp_parameters.LP_NORMALIZATION), getOffset(), true);			
		}		
	}
